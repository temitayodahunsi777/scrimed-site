...
if (process.argv.includes('--self-test')) {
  ...
  (async () => {
      ...
      for (const width of [1440,390]) {
        for (const route of routes) {
          ...
          try {
            ...
            results.push({route,width,failures:['navigation-or-capture-failure'],errorClass:error?.name ?? 'Error'});
    ...
    if (!passed) {
      console.error(JSON.stringify({passed,results},null,2));
      process.exitCode = 1;
    }
  })().catch((error) => { console.error(`Supplemental runner failed: ${error?.name ?? 'Error'}`); process.exitCode = 1; });
}
