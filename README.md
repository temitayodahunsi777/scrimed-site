...
Continuous assurance boundary: `/scrimed-p33` and `/api/scrimed-control-plane/p33/assurance` expose synthetic exact-action policy evaluation, materially independent failover checks, a hard-floor quality ratchet, G21-G25 evidence gates, value-contract readiness, and distinct local/non-PHI/Linux/PHI/production readiness decisions. This does not authorize provider calls, PHI, clinical action, migration, deployment, customer activation, or external distribution.

Adaptive governance boundary: `/scrimed-p34` and `/api/scrimed-control-plane/p34` extend p.33 with provider-neutral capability admission, deterministic-first task routing, hierarchy-aware context provenance, synthetic DICOM privacy manifests, controlled tool stages, contemporaneous governance evidence, two-loop evaluation, task-level FinOps/resilience, hybrid placement, and accountable Agent Operations. Unknown or expired capability denies; high-risk flags remain off. This does not authorize external provider calls, PHI, clinical action, DICOM export, payer submission, EHR/device mutation, migration, deployment, customer activation, certification claims, or external distribution.
