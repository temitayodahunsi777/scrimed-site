...
Workspace hygiene: use `npm run hygiene:workspace` when switching from build/demo work back to implementation work. It clears generated output such as `.next` and TypeScript build info, verifies ignore rules, checks for disposable temp files, and preserves only contract-backed top-level SCRIMED artifacts. It does not remove source modules, docs, migrations, safety gates, or smoke coverage.
