...
begin
  if p_payload ~* '([0-9]{3}-[0-9]{2}-[0-9]{4})'
    ...
    or p_payload ~* 'member[ _-]?(identifier|id)([[:space:]]*[:#][[:space:]]*|[[:space:]]+)[a-z0-9-]{4,}'
    or p_payload ~* 'subscriber[ _-]?(identifier|id)([[:space:]]*[:#][[:space:]]*|[[:space:]]+)[a-z0-9-]{4,}'
    or p_payload ~* 'patient[ _-]?(identifier|id)([[:space:]]*[:#][[:space:]]*|[[:space:]]+)[a-z0-9-]{4,}'
...
comment on function private.reject_execution_attempt_prohibited_text(text) is
  'Rejects credential and identifier-like values with explicit separators while allowing safety metadata labels that name prohibited categories without storing the underlying data.';
