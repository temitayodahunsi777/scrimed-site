...
declare
  ...
  policy_version constant text := 'scrimed-work-review-queue-v2026-07-14';
begin
  select *
...
comment on function private.list_scrimed_work_artifact_review_queue(text, integer) is
  'Reviewer-only AAL2 SCRIMED Work queue. Returns bounded synthetic/no-PHI metadata for artifacts created by another actor and records an audit event.';

comment on function public.list_scrimed_work_artifact_review_queue(text, integer) is
  'Security-invoker wrapper for the tenant-scoped SCRIMED Work reviewer queue. It grants no raw-payload, external distribution, payer, EHR, clinical, connector, certification, or go-live authority.';
