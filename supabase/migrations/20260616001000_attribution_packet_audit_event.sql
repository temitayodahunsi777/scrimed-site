alter table private.pilot_intake_submissions
  add column if not exists last_attribution_analytics_packet_at timestamptz;
...
comment on column private.pilot_intake_submissions.last_attribution_analytics_packet_at is
  'Timestamp of the latest tenant-admin audited attribution analytics packet download for no-PHI sales operations metadata.';

comment on function private.record_sales_artifact_download(text, text, jsonb) is
  'Records tenant-scoped no-PHI sales artifact downloads, including dedicated attribution analytics packet releases, after tenant-admin AAL2 and server-token checks.';
