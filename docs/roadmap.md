...
- Use `/qa-launch-kit`, `/qa-completion-bridge`, and `/qa-proof-promotion` to run the manual Sales Demo Session QA and Authority Reference QA workflows with short-lived AAL2 operator tokens, validate candidate metadata, retain only the no-secret output in `/pilot-workspace/access -> Manual QA Evidence`, confirm packet-hash promotion eligibility, and export Buyer Diligence.
