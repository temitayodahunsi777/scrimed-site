...
- Use `/qa-human-run-packet` for the first approved human AAL2 synthetic dispatch while `/qa-completion-bridge`, protected persistence, `/qa-activation-seal`, `/qa-proof-promotion`, `/qa-claim-guard`, and `/qa-buyer-proof-release` remain the required sequence for packet-backed manual QA evidence.
