...
8. The reviewer enters the assignment ID, reviews the exact packet, and records approval or rejection.
9. The reviewer downloads the short-lived signed evidence JSON through the workspace panel.
10. A release steward supplies that file to `npm run release:scrimed-p32-evidence:strict -- --evidence-file=/absolute/path/file.json` while its outer attestation is current.
11. Remove the transfer file according to the evidence retention policy after the verified gate packet is archived.
...
- Wrong workspace role or inactive reviewer membership: forbidden.
- Same assigning and reviewing identity: forbidden.
- Candidate, packet, assignment, or tenant mismatch: rejected.
