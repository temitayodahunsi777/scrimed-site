...
- Remaining gate: place a CI-held short-lived AAL2 tenant smoke token before using unattended authenticated TrustOps, Agent Workspace, and aggregate enterprise proof-packet happy paths in automated buyer-demo smoke. Public readiness smoke now runs without secrets, while human-run demo readiness and protected verification can be executed from `/pilot-workspace/access` with the current browser session. If password sign-in remains enabled anywhere in the Supabase project, enable leaked-password protection in the dashboard; SCRIMED product flows remain passwordless/passkey-first.
