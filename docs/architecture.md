...
- Protected Operator Metrics: `/pilot-workspace/access` and `/api/pilot-workspaces/[workspaceSlug]/operator-metrics` for AAL2 no-PHI model-cost, review-time, delivery-hour, proof-packet, and workflow-volume capture
- Protected Metric Rollups: `/pilot-workspace/access`, `/api/pilot-workspaces/[workspaceSlug]/metric-rollups`, and `/api/pilot-workspaces/[workspaceSlug]/metric-rollups/[snapshotId]/packet` for AAL2 no-PHI internal board operating snapshots and audited packet downloads
...
- Protected Release Decision Workflow: `/pilot-workspace/access`, `/api/pilot-workspaces/[workspaceSlug]/release-decisions`, and `/api/pilot-workspaces/[workspaceSlug]/release-decisions/packet` for no-PHI versioned claim registry decisions that require linked external approval domains before qualified release review
- Protected Named Reviewer Sign-Off Packets: `/pilot-workspace/access`, `/api/pilot-workspaces/[workspaceSlug]/reviewer-signoffs`, and `/api/pilot-workspaces/[workspaceSlug]/reviewer-signoffs/packet` for no-PHI metadata references to externally retained reviewer sign-offs before controlled distribution review
- Protected Distribution Lockbox: `/pilot-workspace/access`, `/api/pilot-workspaces/[workspaceSlug]/distribution-lockbox`, and `/api/pilot-workspaces/[workspaceSlug]/distribution-lockbox/packet` for disabled-by-default no-PHI distribution metadata linked to external approvals, customer permission references, counsel review references, reviewer sign-offs, and artifact manifests
- Protected Release Authority Attestations: `/pilot-workspace/access`, `/api/pilot-workspaces/[workspaceSlug]/release-authority-attestations`, and `/api/pilot-workspaces/[workspaceSlug]/release-authority-attestations/packet` for metadata-only references to externally retained release authority while release remains disabled
- Protected Evidence Room Recipient Attestations: `/pilot-workspace/access`, `/api/pilot-workspaces/[workspaceSlug]/evidence-room-recipient-attestations`, and `/api/pilot-workspaces/[workspaceSlug]/evidence-room-recipient-attestations/packet` for metadata-only intended recipient segment, access-window, packet-reference, and revocation controls while exact recipient lists and export authority remain external
- Protected Evidence Room Access Log Reconciliation: `/pilot-workspace/access`, `/api/pilot-workspaces/[workspaceSlug]/evidence-room-access-log-reconciliation`, and `/api/pilot-workspaces/[workspaceSlug]/evidence-room-access-log-reconciliation/packet` for metadata-only references to externally retained evidence-room access logs while raw logs, recipient identifiers, and export authority remain external
- Protected Evidence Room Provider Adapter Contracts: `/pilot-workspace/access`, `/api/pilot-workspaces/[workspaceSlug]/evidence-room-provider-adapters`, and `/api/pilot-workspaces/[workspaceSlug]/evidence-room-provider-adapters/packet` for metadata-only provider contract references and audit-log import stubs while provider credentials, URLs, tokens, raw logs, live integration approval, and export authority remain external
...
- The protected Clinical Activation Approval Workflow persists append-only AAL2 no-PHI readiness attestations for regulatory classification, clinical governance and safety, privacy/security/PHI readiness, interoperability connector validation, legal/commercial/reimbursement boundary, and go-live/rollback/operations while live clinical execution remains blocked.
- Protected Operator Metrics persist tenant-scoped no-PHI operating metric captures for Public Market Readiness, unit-economics discipline, proof-stack coverage, pricing posture, and protected Metric Rollups while audited financial reporting and securities use remain blocked.
- Protected Metric Trend Reviews compare finance-reviewed no-PHI rollup snapshots for monthly variance review, reach expansion planning, competitive advantage tracking, and Agent Commander improvement priorities while audited financial reporting, securities use, clinical validation, and live care remain blocked.
- Protected Board Scorecards package one to three no-PHI trend reviews into rolling-quarter operating decisions, finance-allocation readiness, buyer-segment cohorts, competitive advantage tracking, and Agent Commander priorities while full allocation methodology, audited reporting, securities use, clinical validation, advertising substantiation, and live care remain blocked.
- Protected Finance Methodology Gates convert scorecard limitations into explicit internal gate records for finance, counsel, executive release, privacy/security, clinical boundary, marketing claims, and buyer permission while external reporting, securities use, advertising claims, clinical validation, reimbursement assurance, customer case studies, and live care remain blocked.
- Protected Release Decision Workflow converts external approval metadata references into versioned claim registry decisions for buyer, investor, PR, site, collateral, board, and case-study review while public release, legal approval, advertising substantiation, customer permission, securities use, clinical validation, production authorization, and live care remain blocked.
- Protected Named Reviewer Sign-Off Packets convert release decisions into role-coverage metadata for finance, counsel, executive, privacy/security, clinical-governance, marketing, and buyer-permission reviewers while public release, external distribution, legal approval, advertising substantiation, customer permission, securities use, clinical validation, production authorization, and live care remain blocked.
- Protected Distribution Lockbox converts named reviewer sign-off coverage into disabled distribution metadata for buyer, investor, board, sales, marketing, PR, and customer-proof channels while public release, external distribution, legal approval, advertising substantiation, customer permission, securities use, clinical validation, production authorization, and live care remain blocked.
- Protected Release Authority Attestations convert ready disabled lockboxes into release-authority domain metadata coverage for counsel, customer permission, executive, privacy/security, finance, clinical governance, and marketing claims while public release, external distribution, legal approval, advertising substantiation, audited reporting, securities use, clinical validation, reimbursement assurance, production authorization, and live care remain blocked.
- Protected Evidence Room Recipient Attestations convert completed release-authority attestations into intended recipient-scope, access-window, packet-reference, and revocation metadata while exact recipient lists, recipient emails, access grants, public release, external distribution, legal approval, audited reporting, securities use, clinical validation, reimbursement assurance, production authorization, and live care remain blocked.
- Protected Evidence Room Access Log Reconciliation converts recipient attestations into externally retained access-log references, reconciliation windows, event-count summaries, anomaly posture, and revocation review while raw logs, recipient identifiers, access grants, public release, external distribution, legal approval, audited reporting, securities use, clinical validation, reimbursement assurance, production authorization, and live care remain blocked.
- Protected Evidence Room Provider Adapter Contracts convert completed access-log reconciliation into metadata-only provider contract references, disabled adapter modes, audit-log import stub labels, summary format readiness, verification cadence, and provider risk posture while provider credentials, URLs, tokens, raw logs, signed legal artifacts, live integration approval, export authority, public release, clinical validation, and live care remain blocked.
...
- Persistent Agent Workspace v1 replaces vague long-running agent promises with inspectable synthetic work orders, model-router policies, reviewer checkpoints, audit timelines, blocked actions, downloadable work-order proof packets, and tenant-admin aggregate enterprise proof packets.
