...
- QA Evidence Ledger, Manual AAL2 Activation Plan, Execution Readiness, Run Control, and Proof Promotion
...
Use `/qa-run-control` to execute the first deliberate short-lived human AAL2 QA workflow with a no-secret operator brief, persist the safe packet metadata through the protected workspace, confirm `/qa-proof-promotion`, then link that retained proof into Buyer Pilot Room and Boundary Resolution evidence.
