...
function buildGateRegistry({ validationPassed, controlsPassed, immutableCandidate, generatedAt, expiresAt }) {
  const gates = [
    ...
    ["supabase-passwordless-protected-access", "COMPENSATING_CONTROL_ACTIVE", "security-identity-owner", "Passwordless entry, disabled signup/user creation, AAL2, short sessions, rate limits, tenant/role checks, and server authorization protect the current synthetic lane."],
    ["supabase-leaked-password-protection", "DEFERRED_PLATFORM_CONTROL", "supabase-owner", "The current plan does not expose this control. The warning remains open and password-based protected auth is denied until the control is verified."],
    ["supabase-password-auth-invariant", "PASS_SAFE_BOUNDARY_ENFORCED", "security-identity-owner", "Any protected password-auth path without verified leaked-password protection is denied."],
