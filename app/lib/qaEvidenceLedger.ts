...
const qaManualExecutionConsoleRoute = "/qa-manual-execution-console";
const qaManualExecutionConsoleApiRoute = "/api/qa-evidence/manual-execution-console";
const qaManualExecutionConsoleBriefRoute =
...
export const qaEvidenceEntries: QaEvidenceEntry[] = [
  ...
  {
    id: "manual-aal2-qa-run-control",
    name: "Manual AAL2 QA run control",
    status: "workaround-active",
    owner: "Release engineering and TrustOS",
    recordedAt: "2026-06-21",
    artifact: "/api/qa-evidence/run-control",
    routes: [
      "/qa-run-control",
      "/api/qa-evidence/run-control",
      "/api/qa-evidence/run-control/brief",
    ...
    limitation:
      "Run Control prepares the first authenticated synthetic QA run; it still does not execute passkey ceremonies, mint tokens, store credentials, or claim retained authenticated evidence.",
    workaround:
  ...
  {
    id: "manual-aal2-qa-human-run-packet",
    name: "Manual AAL2 QA human run packet",
    status: "workaround-active",
    owner: "Release engineering, TrustOS, security, tenant governance, and Buyer Diligence",
    recordedAt: "2026-06-22",
    artifact: "/api/qa-evidence/human-run-packet",
    routes: [
      "/qa-human-run-packet",
    ...
    limitation:
      "The Human Run Packet still does not execute passkey ceremonies, mint tokens, run workflows, persist packet hashes, authorize production data, or claim retained authenticated proof.",
    workaround:
      "Use the packet to make the first human AAL2 run deterministic and auditable while protected Manual QA Evidence remains the only source of retained proof.",
    nextAction:
      "Have an approved tenant-admin or pilot-lead use /qa-human-run-packet for exactly one synthetic workflow, validate through /qa-completion-bridge, persist protected metadata, then confirm /qa-activation-seal, /qa-proof-promotion, and /qa-claim-guard."
  },
  {
    id: "manual-aal2-qa-execution-console",
    name: "Manual AAL2 QA execution console",
    status: "workaround-active",
    owner: "Release engineering, TrustOS, tenant governance, security, and Buyer Diligence",
    recordedAt: "2026-06-22",
    artifact: qaManualExecutionConsoleApiRoute,
    routes: [
    ...
    limitation:
      "The console coordinates execution but does not mint tokens, execute passkey ceremonies, run authenticated CI unattended, store secrets, process PHI, create retained packets by itself, or authorize production healthcare activity.",
    workaround:
      "Use the public route for the no-secret runbook and the protected workspace panel to read retained packet and audit visibility before any Buyer Diligence proof language is released.",
    nextAction:
  ...
  },
  {
    ...
    recordedAt: "2026-06-22",
    artifact: "/api/qa-evidence/completion-bridge",
    routes: [
      "/qa-completion-bridge",
      "/api/qa-evidence/completion-bridge",
      "/api/qa-evidence/completion-bridge/brief",
      qaManualRunEvidencePacketApiRoute,
      qaManualRunEvidencePersistenceApiRoute,
      "/qa-proof-promotion"
    ],
    ...
    limitation:
      "The bridge does not execute the human AAL2 run, store evidence, store tokens, create protected packet hashes, or authorize clinical, PHI, reimbursement, security-certification, connector, or production claims.",
    workaround:
      "Use the bridge immediately after the human Launch Kit workflow to reject unsafe candidate metadata before submitting the same no-secret payload through protected Manual QA Evidence persistence.",
    nextAction:
  ...
  {
    id: "manual-aal2-qa-activation-seal",
    name: "Manual AAL2 QA activation seal",
    status: "workaround-active",
    owner: "TrustOS, release engineering, tenant governance, buyer diligence, and claims governance",
    recordedAt: "2026-06-22",
    artifact: "/api/qa-evidence/activation-seal",
    routes: [
      "/qa-activation-seal",
      "/api/qa-evidence/activation-seal",
      "/api/qa-evidence/activation-seal/brief",
      "/qa-launch-kit",
      "/qa-completion-bridge",
      "/qa-claim-guard",
      "/qa-proof-promotion",
    ...
    limitation:
      "The Activation Seal does not execute AAL2 workflows, create protected packet hashes, store tokens, store PHI, certify security or compliance, authorize live clinical care, guarantee reimbursement, approve production connectors, or create public release authority.",
    workaround:
      "Use Activation Seal after protected Manual QA Evidence persistence and before Buyer Diligence export so retained packet hash, audit event, Proof Promotion, Claim Guard, and blocked production-authority language are checked together.",
    nextAction:
      "After the first human AAL2 workflow is persisted, verify /qa-activation-seal, /qa-proof-promotion, and /qa-claim-guard before using packet-backed buyer proof language."
  },
  {
...
export function getQaEvidenceLedger() {
  ...
  return {
    ...
    qaManualExecutionConsole: {
      ...
      boundary:
        "The console coordinates human AAL2 synthetic QA, retained packet visibility, and Buyer Proof Release checks without storing tokens, PHI, production credentials, or clinical authority."
    },
