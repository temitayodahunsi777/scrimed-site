...
export async function POST(
  ...
  const result = await guardedTransitionSession(request, sessionId, "resume", "Protected resume requested after checkpoint review.");
  ...
  return NextResponse.json(wrapWorkData(result.data, sessionId), {
    headers: scrimedWorkHeaders({ "X-SCRIMED-Session-Control": "resumed-aal2-durable-write" }, request)
  });
}
