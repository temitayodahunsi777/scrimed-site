...
import {
  buildProtectedEvidenceRoomProviderAdapterPacket,
  ...
  protectedEvidenceRoomProviderAdapterStorageAuthority
...
import {
  ...
  recordProtectedEvidenceRoomProviderAdapterPacketDownload
...
export async function GET(request: Request, { params }: RouteContext) {
  const rateLimit = await enforceRequestRateLimit(request, {
    namespace: "protected-evidence-room-provider-adapter-packet-download",
    limit: 12,
    windowSeconds: 600
  });
    ...
    return NextResponse.json(
      {
        error: {
          ...
          message: "Protected evidence-room provider adapter packet downloads are temporarily rate limited."
        },
  ...
  const auditResult = await recordProtectedEvidenceRoomProviderAdapterPacketDownload(
    context.client,
    workspace.slug,
    {
      adapterCount: workflow.summary.adapterCount,
      readyForReviewCount: workflow.summary.readyForReviewCount,
      exportDisabledCount: workflow.summary.exportDisabledCount,
      rawLogImportDisabledCount: workflow.summary.rawLogImportDisabledCount,
      credentialStorageDisabledCount: workflow.summary.credentialStorageDisabledCount,
      requiredProviderControlCount: workflow.summary.requiredProviderControlCount,
      linkedProviderControlCount: workflow.summary.linkedProviderControlCount,
      missingProviderControlCount: workflow.summary.missingProviderControlCount,
      readyAccessLogReconciliationCount: workflow.summary.readyAccessLogReconciliationCount,
      providerRiskReviewCount: workflow.summary.providerRiskReviewCount,
      adapterState: workflow.adapterState,
  ...
  if (auditResult.error || !auditResult.eventId) {
    ...
    return NextResponse.json(
      {
        error: {
          code: "protected-evidence-room-provider-adapter-packet-audit-failed",
          message:
            "The protected evidence-room provider adapter packet was not released because the audit event could not be committed."
        },
  ...
  const packet = buildProtectedEvidenceRoomProviderAdapterPacket({
  ...
  return new NextResponse(packet, {
    headers: {
      ...headers,
      "Content-Disposition": `attachment; filename="scrimed-${safeWorkspaceSlug}-evidence-room-provider-adapters.md"`,
      "Content-Type": "text/markdown; charset=utf-8",
      "X-SCRIMED-Proof-Stack": protectedEvidenceRoomProviderAdapterPacketProofStackStatus,
      "X-SCRIMED-Provider-Adapter-Authority": protectedEvidenceRoomProviderAdapterAuthority,
      "X-SCRIMED-Release-Authority": protectedEvidenceRoomProviderAdapterReleaseAuthority,
      "X-SCRIMED-Storage-Authority": protectedEvidenceRoomProviderAdapterStorageAuthority,
